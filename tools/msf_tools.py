...
@tool
def get_msf_sub_groups_list(db_url: str = 'sqlite:///metasploit_data.db') -> List[str]:
    """
    ...
    try:
        ...
        sub_groups = manager_db.get_sub_group_from_module_auxiliary()
        return sub_groups
...
@tool
def get_msf_exact_sub_group_modules_list(db_url: str, sub_group_name: str) -> List[Tuple[str, str]]:
    """
    ...
    try:
        manager_db = ManagerAlchemyDB(db_url)
        modules = manager_db.get_modules_by_sub_group_list_from_module_auxiliary(sub_group_name)
        return modules
    except Exception as e:
...
def msf_console_scan_tool_dynamic(input_dict: Any) -> str:
    ...
    try:
        ...
        args = _process_input_args(input_dict)
